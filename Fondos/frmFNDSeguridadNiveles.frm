...
Begin VB.Form frmFNDSeguridadNiveles 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Fondos: Niveles de Seguridad (Retiros y Liquidaciones)"
   ClientHeight    =   8670
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   10785
   ...
   Begin FPSpreadADO.fpSpread vGrid 
      ...
      MaxCols         =   494
      ScrollBars      =   2
      SpreadDesigner  =   "frmFNDSeguridadNiveles.frx":0000
   ...
   Begin VB.Label Label1 
      ...
      Caption         =   "Niveles de Seguridad: Retiros y Liquidaciones"
      BeginProperty Font 
         Name            =   "Calibri"
         Size            =   13.5
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmFNDSeguridadNiveles"
...
strSQL = "select Pl.cod_Operadora, Pl.cod_Plan,Pl.Descripcion,Asg.registro_Fecha,ASg.Registro_Usuario" _
       & " from Fnd_Planes Pl left join FND_SEGURIDAD_PLANES Asg on Pl.cod_operadora = Asg.cod_Operadora" _
       & " and Pl.cod_Plan = Asg.Cod_Plan and Asg.COD_GRUPO = '" & scNivel.Tag _
...
strSQL = "select Us.Nombre,Us.Descripcion,Asg.registro_Fecha,ASg.Registro_Usuario" _
       & " from Usuarios Us left join FND_SEGURIDAD_USUARIOS Asg on Us.Nombre = Asg.Usuario" _
       & " and Asg.COD_GRUPO = '" & scNivel.Tag _
...
strSQL = "select * from FND_SEGURIDAD_GRUPOS" _
      & " order by COD_GRUPO"
...
Do While Not rs.EOF
  ...
  For i = 1 To vGrid.MaxCols
    ...
    Select Case i
     ...
     Case 3 'Inicio
        vGrid.Text = Format(rs!Monto_Inicio, "Standard")
     Case 4 'Corte
        vGrid.Text = Format(rs!Monto_Corte, "Standard")
     Case 5 ' Activo
...
vGrid.col = 1
strSQL = "select isnull(count(*),0) as Existe from FND_SEGURIDAD_GRUPOS " _
       & " where COD_GRUPO = '" & vGrid.Text & "'"
Call OpenRecordSet(rs, strSQL)

If rs!Existe = 0 Then 'Insertar

  strSQL = "insert into FND_SEGURIDAD_GRUPOS(COD_GRUPO,descripcion,monto_inicio,Monto_Corte,activo, registro_fecha, registro_usuario)" _
         & " values('" & vGrid.Text & "','"
  vGrid.col = 2
  ...
  vGrid.col = 1
  Call Bitacora("Registra", "Grupo de Seguridad: " & vGrid.Text)

Else 'Actualizar

 vGrid.col = 2
 strSQL = "update FND_SEGURIDAD_GRUPOS set descripcion = '" & vGrid.Text & "', Monto_Inicio = "
 vGrid.col = 3
 ...
 vGrid.col = 1
 Call Bitacora("Modifica", "Grupo de Seguridad: " & vGrid.Text)
...
If Item.Checked Then
   strSQL = "insert FND_SEGURIDAD_PLANES(cod_operadora,cod_plan,COD_GRUPO,registro_usuario,registro_fecha)" _
          & " values(" & Item.Tag & ",'" & Item.Text & "','" & vCodigo & "','" & glogon.Usuario & "',dbo.MyGetdate())"
   Item.SubItems(2) = glogon.Usuario
   Item.SubItems(3) = Date

Else
   strSQL = "delete FND_SEGURIDAD_PLANES where cod_operadora  = " & Item.Tag & " and  cod_Plan = '" & Item.Text _
          & "' and COD_GRUPO = '" & vCodigo & "'"
...
If Item.Checked Then
   strSQL = "insert FND_SEGURIDAD_USUARIOS(usuario,COD_GRUPO,registro_usuario,registro_fecha)" _
          & " values('" & Item.Text & "','" & vCodigo & "','" & glogon.Usuario & "',dbo.MyGetdate())"
   Item.SubItems(2) = glogon.Usuario
   Item.SubItems(3) = Date

Else
   strSQL = "delete FND_SEGURIDAD_USUARIOS where usuario = '" & Item.Text _
          & "' and COD_GRUPO = '" & vCodigo & "'"
...
If KeyCode = vbKeyDelete Then
     ...
     If i = vbYes Then
        vGrid.Row = vGrid.ActiveRow
        vGrid.col = 1
        strSQL = "delete FND_SEGURIDAD_GRUPOS where COD_GRUPO = '" & vGrid.Text & "'"
        Call ConectionExecute(strSQL)

        strSQL = vGrid.Text
        vGrid.col = 1
        Call Bitacora("Elimina", "Grupo de Seguridad: " & vGrid.Text)
