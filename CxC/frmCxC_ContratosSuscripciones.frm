...
Begin VB.Form frmCxC_ContratosSuscripciones 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   ...
   End
End
Attribute VB_Name = "frmCxC_ContratosSuscripciones"
...
Dim vTasaCor As Currency, vTasaMor As Currency, vPlazo As Long, vComision As Currency
...
If vScroll Then
    strSQL = "select Top 1 cedula from CxC_Personas_Contratos"
    
    If FlatScrollBar.Value = 1 Then
       strSQL = strSQL & " where cod_contrato = '" & txtCodigo.Text & "' and cedula > '" & txtCedula.Text & "' order by cedula asc"
    Else
       strSQL = strSQL & " where cod_contrato = '" & txtCodigo.Text & "' and cedula < '" & txtCedula.Text & "' order by cedula desc"
    End If
...
strSQL = "select *,dbo.MyGetdate() as 'Fecha' from CxC_Contratos where cod_contrato = '" & GLOBALES.gTag & "'"
Call OpenRecordSet(rs, strSQL)
...
strSQL = "select P.nombre,C.* " _
       ...
       & " where C.cedula = '" & pCedula & "' and C.cod_contrato = '" & txtCodigo.Text & "'"
Call OpenRecordSet(rs, strSQL)
If Not rs.BOF And Not rs.EOF Then
  Call sbToolBar(Me.tlb, "activo")
  vEdita = True
  vCodigo = Trim(rs!Cedula)
...
Else
  'Busca Datos del Cliente Unicamente
    strSQL = "select nombre from CxC_Personas" _
...
If vEdita Then
  strSQL = "Update CxC_Personas_Contratos Set Cedula ='" & Trim(txtCedula.Text) & "', Notas = '" & Trim(txtNotas.Text) _
         ...
         & "' where cedula = '" & vCodigo & "' and cod_contrato = '" & txtCodigo.Text & "'"
  Call ConectionExecute(strSQL)
  
  
  Call Bitacora("Modifica", "Suscripción: Ced." & vCodigo & " Cnt: " & Trim(txtCodigo.Text))
...
Else
   ...
   Call Bitacora("Registra", "Suscripción: Ced." & vCodigo & " Cnt: " & Trim(txtCodigo.Text))
   
End If

vCodigo = Trim(txtCedula.Text)
vEdita = True
...
If i = vbYes Then
  strSQL = "delete CxC_Personas_Contratos where cedula = '" & vCodigo & "' and cod_contrato = '" & txtCodigo.Text & "'"
  Call ConectionExecute(strSQL)
  Call Bitacora("Elimina", "Suscripción: Ced." & vCodigo & " Cnt: " & Trim(txtCodigo.Text))
...
If Trim(txtCedula.Text) <> "" Then
   Call sbConsulta(Trim(txtCedula))
End If
