...
Begin VB.Form frmSeguros_TiposPolizas 
   Appearance      =   0  'Flat
   BackColor       =   &H80000005&
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Tipos de Seguros (Productos)"
   ClientHeight    =   7605
   ClientLeft      =   45
   ClientTop       =   315
   ClientWidth     =   13455
   ...
   Begin XtremeSuiteControls.TabControl tcMain 
      ...
      Begin FPSpreadADO.fpSpread vGrid 
         ...
         MaxCols         =   499
         ScrollBars      =   2
         SpreadDesigner  =   "frmSeguros_TiposPolizas.frx":0000
      ...
      Begin FPSpreadADO.fpSpread vGridc 
         ...
         MaxCols         =   6
         ScrollBars      =   2
         SpreadDesigner  =   "frmSeguros_TiposPolizas.frx":07F4
         VScrollSpecialType=   2
         AppearanceStyle =   1
      End
      Begin XtremeSuiteControls.ComboBox cboSeguro 
         Height          =   330
         Left            =   -67720
         TabIndex        =   7
         Top             =   480
      ...
      End
      Begin VB.Label Label2 
         Caption         =   "Coberturas Relacionadas?"
         BeginProperty Font 
            Name            =   "Arial Narrow"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmSeguros_TiposPolizas"
...
Call sbProductos_List
End Sub


Private Sub cboSeguro_Click()
...
If vPaso Then Exit Sub
If cboAseguradora.ListCount = 0 Then Exit Sub
If cboSeguro.ListCount = 0 Then Exit Sub


strSQL = "exec spSeguros_Productos_Coberturas'" & cboAseguradora.ItemData(cboAseguradora.ListIndex) & "','" & cboSeguro.ItemData(cboSeguro.ListIndex) & "'"
...
strSQL = "select isnull(count(*),0) as Existe from SEGUROS_TIPOS_PRODUCTOS " _
...
If rs!Existe = 0 Then 'Insertar
  ...
  strSQL = "Insert SEGUROS_TIPOS_PRODUCTOS(COD_ASEGURADORA,COD_PRODUCTO,descripcion,CODIGO_RETENCION, COD_PRODUCTO_ASEGURADORA,COD_FORMATO" _
         & ", Comision_Porc,COMISION_PORC_PRICTA,Activo,Registro_Usuario,Registro_Fecha)" _
  ...
  vGrid.Col = 1
  Call Bitacora("Registra", "Tipo de Seguro:" & vGrid.Text & "_" & cboAseguradora.ItemData(cboAseguradora.ListIndex))

Else 'Actualizar

 vGrid.Col = 2
 strSQL = "update SEGUROS_TIPOS_PRODUCTOS set descripcion = '" & vGrid.Text & "',CODIGO_RETENCION  = '"
 vGrid.Col = 3
 strSQL = strSQL & vGrid.Text & "', COD_PRODUCTO_ASEGURADORA= '"
 vGrid.Col = 4
 strSQL = strSQL & vGrid.Text & "', COD_FORMATO = '"
 vGrid.Col = 5
 ...
 vGrid.Col = 8
 strSQL = strSQL & vGrid.Value & " where COD_PRODUCTO = '"
 vGrid.Col = 1
 strSQL = strSQL & vGrid.Text & "' AND COD_ASEGURADORA = '" & cboAseguradora.ItemData(cboAseguradora.ListIndex) & "'"
 Call ConectionExecute(strSQL)

 vGrid.Col = 1
 Call Bitacora("Modifica", "Tipo de Seguro:" & vGrid.Text & "_" & cboAseguradora.ItemData(cboAseguradora.ListIndex))
...
Private Sub sbProductos_List()
Dim strSQL As String

On Error GoTo vError
Me.MousePointer = vbHourglass

   strSQL = "select COD_PRODUCTO,descripcion,CODIGO_RETENCION,COD_PRODUCTO_ASEGURADORA,COD_FORMATO,comision_porc,COMISION_PORC_PRICTA,Activo" _
...
Case 0 'Tipos de Productos
  Call sbProductos_List
    
Case 1 'Coberturas Relacionadas
   strSQL = "select rtrim(COD_PRODUCTO) as 'IdX', rtrim(descripcion) as 'ItmX'" _
          & " from SEGUROS_TIPOS_PRODUCTOS" _
          & " where cod_aseguradora = '" & cboAseguradora.ItemData(cboAseguradora.ListIndex) & "'" _
          & " order by COD_PRODUCTO"
   vPaso = True
   Call sbCbo_Llena_New(cboSeguro, strSQL, False, True)
   vPaso = False
   
   
   Call cboSeguro_Click
...
If KeyCode = vbKeyDelete Then
     ...
     If i = vbYes Then
        vGrid.Row = vGrid.ActiveRow
        vGrid.Col = 1
        strSQL = "delete SEGUROS_TIPOS_PRODUCTOS" _
               & " where COD_PRODUCTO = '" & vGrid.Text & "' and cod_Aseguradora = '" & cboAseguradora.ItemData(cboAseguradora.ListIndex) & "'"
        Call ConectionExecute(strSQL)
        
        strSQL = vGrid.Text
        vGrid.Col = 1
        Call Bitacora("Elimina", "Tipo de Seguro:" & vGrid.Text & "_" & cboAseguradora.ItemData(cboAseguradora.ListIndex))
...
vProducto = cboSeguro.ItemData(cboSeguro.ListIndex)
