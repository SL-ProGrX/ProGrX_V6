...
If IsNumeric(txtSesion.Text) Then
       If vWhere Then
            strSQL = strSQL & " AND "
       Else
            strSQL = strSQL & " WHERE "
       ...
       strSQL = strSQL & " ID_SESION = " & txtSesion.Text
...
Select Case cboEstado.Text
    ...
    Case "Inactivos"
       ...
       Else
            ...
            vWhere = True
       End If
       strSQL = strSQL & " ESTADO = 'I'"
    
    Case "Pendiente Actualizar"
       If vWhere Then
            strSQL = strSQL & " AND "
       Else
            strSQL = strSQL & " WHERE "
            vWhere = True
       End If
       strSQL = strSQL & " ESTADO = 'A' AND REGISTRO_FECHA IS NOT NULL AND ACTUALIZA_FECHA is null"
    
    
    Case "Pendiente Imprimir"
       If vWhere Then
            strSQL = strSQL & " AND "
       Else
            strSQL = strSQL & " WHERE "
            vWhere = True
       End If
       strSQL = strSQL & " ESTADO = 'A' AND REGISTRO_FECHA IS NOT NULL AND IMPRIME_FECHA is null"
End Select
...
Do While Not rs.EOF
 Set itmX = lsw.ListItems.Add(, , rs!ID_ROE)
     ...
     If Not IsNull(rs!Registro_Fecha) Then
        If rs!Estado = "A" And IsNull(rs!Imprime_fecha) Then
            itmX.TextBackColor = RGB(247, 244, 210)
        End If
        
        If rs!Estado = "I" Then
            itmX.TextBackColor = RGB(243, 185, 176)
        End If
     End If
...
For i = 1 To .Count
    If .Item(i).Checked Then
        ...
        strSQL = "select dbo.fxCajas_ROE_Imprime_Valida(" & .Item(i).Text & ") as 'Imprime'"
        Call OpenRecordSet(rs, strSQL)
        
        If rs!Imprime = 1 Then
           Call sbCajas_ROE(.Item(i).Text)
        Else
           MsgBox "Este Documento no ha sido actualizado/revisado. Verifique los datos, guarde y luego imprima! ROE Id: " & .Item(i).Text, vbExclamation
        End If
...
cboEstado.AddItem "Pendiente Actualizar"
cboEstado.AddItem "Pendiente Imprimir"
cboEstado.Text = "Pendiente Imprimir"
