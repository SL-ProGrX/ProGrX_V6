...
If (Len(Trim(txtExpediente.Text)) = 0) And (Len(Trim(txtNombre.Text)) = 0) And (Len(Trim(txtCedula.Text)) = 0) Then
   ' MsgBox "La consulta que intenta realizar es una consulta muy general. Es necesario que introduzca por lo menos un criterio para la búsqueda.", vbInformation, gTituloMsg
...
End If
    ...
    If clsEntidad.fxTraerFiltrado("Criterios", fxAgregaColleccion(txtExpediente.Text & "%", "%" & txtNombre.Text & "%", txtCedula.Text & "%")) Then
        Call sbCargaLista
    Else
        MsgBox "No se encontró ningún asegurado con los criterios especificados.", vbInformation, gTituloMsg
    End If
